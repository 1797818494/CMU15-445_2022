...
class BufferPoolManagerInstance : public BufferPoolManager {
 ...
 protected:
  ...
  auto IsFullPage() const -> bool { return free_list_.empty() && replacer_->Size() == 0; }
  void GetListFront(frame_id_t *frame_id) {
    *frame_id = free_list_.front();
    free_list_.pop_front();
    page_table_->Remove(pages_[*frame_id].page_id_);
    replacer_->Remove(*frame_id);
  }
  void GetAndEvict(frame_id_t *frame_id) {
  ...
  auto AllocateNewFrame(frame_id_t frame_id, page_id_t *page_id) -> Page * {
    ...
    *page_id = pages_[frame_id].page_id_;
    ...
    // printf("new page pageid %d  frameid %d\n", *page_id, frame_id);
