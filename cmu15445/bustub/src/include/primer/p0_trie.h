 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (i == n - 1) {
        if (iter == nullptr) {
          (**parent).InsertChildNode(cur, std::make_unique<TrieNodeWithValue<T>>(key[i], value));
      ...
      if (iter == nullptr) {
        parent = (**parent).InsertChildNode(cur, std::make_unique<TrieNode>(key[i]));
      } else {
        parent = iter;
      }
    }
  ...
  bool Remove(const std::string &key, TrieNode &parent) {
    ...
    if (!(**iter).HasChildren() && flag && !(**iter).IsEndNode()) {
      parent.RemoveChildNode(key[0]);
    }
