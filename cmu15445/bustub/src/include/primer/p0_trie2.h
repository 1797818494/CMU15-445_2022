...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    auto iter = children_.find(key_char);
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (int i = 0; i < n; i++) {
      char cur = key[i];
      auto iter = (*parent)->GetChildNode(cur);
      if (i == n - 1) {
        if (iter == nullptr) {
          (**parent).InsertChildNode(cur, std::unique_ptr<TrieNode>(new TrieNodeWithValue<T>(key[i], value)));
          //(*flag).reset(new TrieNodeWithValue<T>(std::move(**flag), value));
          latch_.WUnlock();
          return true;
        }
        bool flag = (*iter)->IsEndNode();
        if (!flag) {
          (*iter).reset(new TrieNodeWithValue<T>(std::move(**iter), value));
        }
        latch_.WUnlock();
        return false;
      }
      if (iter == nullptr) {
        parent = (**parent).InsertChildNode(cur, std::unique_ptr<TrieNode>(new TrieNode(key[i])));
      } else {
        parent = iter;
      }
    }
   ...
   */
  bool Remove(const std::string &key, TrieNode &parent) {
    if (key.size() == 1) {
      if (parent.HasChild(key[0]) && (*(parent.GetChildNode(key[0])))->IsEndNode()) {
        auto tmp = parent.GetChildNode(key[0]);
        (**tmp).SetEndNode(false);
        if ((**tmp).HasChildren() == false) {
          parent.RemoveChildNode(key[0]);
        }
        return true;
      } else {
        return false;
    ...
    auto iter = parent.GetChildNode(key[0]);
    if (iter == nullptr) return false;
    bool flag = Remove(key.substr(1), **iter);
    if ((**iter).HasChildren() == false && flag == true && (**iter).IsEndNode() == false) {
      parent.RemoveChildNode(key[0]);
    }
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (iter == nullptr) {
        ...
        *success = false;
        ...
        if (i == n - 1) {
          bool flag1 = (**iter).IsEndNode();
          if (flag1 == false) {
            latch_.RUnlock();
            *success = false;
            return {};
          }
          auto flag = dynamic_cast<TrieNodeWithValue<T> *>((*iter).get());
          if (flag != nullptr) {
            latch_.RUnlock();
            *success = true;
